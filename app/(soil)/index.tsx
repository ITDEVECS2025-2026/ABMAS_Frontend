// Lokasi: app/(tabs)/abmas/index.tsx
...
import { MqttProps } from "@/interfaces/IMqtt";
import { useNormalizedURL } from "@/utils/useURLConvertion";
import { VStack } from "@/components/ui/vstack";
import { useAtom } from "jotai";

// Import semua atom (termasuk yang baru)
import {
  mqqtMessageAtom,
  mqttIsConnectedAtom,
...
// Skema validasi (tetap di sini karena hanya dipakai oleh form ini)
...
export default function Index() {
  ...
  // Baca state global dari Jotai
  const [isConnected, setIsConnected] = useAtom(mqttIsConnectedAtom);
  const [connectionStatus, setConnectionStatus] = useAtom(mqttConnectionStatusAtom);
  const [subscribedTopics] = useAtom(mqttSubscribedTopicsAtom);
  const [receivedMessages, setReceivedMessages] = useAtom(mqqtMessageAtom);

  // State lokal khusus untuk UI ini
  ...
  // useEffect sudah dipindah ke _layout.tsx
  ...
  const handleConnect = (data: MqttProps) => {
    if (!isConnected) {
      ...
      mqtt.connect(brokerUrl, {
        ...
        .then(() => {
          // Kita tetap set state di sini untuk respons instan
          setIsConnected(true);
      ...
      setReceivedMessages([]);
      mqtt.disconnect();
      // State akan diupdate oleh listener 'onDisconnected'
    }
  };
  ...
  // --- JSX (UI) Anda dimulai di sini ---
  // (Tidak ada perubahan pada JSX, cukup copy-paste)
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerSubtitle}>
          Sistem Pemantauan Pertanian Lanjutan
        </Text>
      </View>
        ...
        <Form<MqttProps> methods={methods}>
          ...
          >
            <ButtonText style={styles.connectButtonText}>
              {methods.formState.isSubmitting || clickedDelay
                ? "🔄 Menghubungkan..."
                : isConnected
                ? "🔌Putuskan"
                : "⚡ Hubungkan"}
      ...
      {isConnected && (
        <View style={styles.controlPanel}>
          ...
          <View style={styles.buttonRow}>
            ...
            >
              <ButtonText style={styles.controlButtonText}>
                📤 Test Koneksi
              </ButtonText>
            </Button>
            ...
            {receivedMessages.length === 0 ? (
              <View style={styles.noMessagesContainer}>
                ...
                <Text style={styles.noMessagesSubtext}>
                  Hubungkan untuk mulai menerima data
                </Text>
              </View>
            ) : (
...
// --- STYLESHEET ---
// (Copy-paste seluruh blok 'const styles = StyleSheet.create({...}' 
// dari file asli Anda ke sini. Tidak ada perubahan.)
